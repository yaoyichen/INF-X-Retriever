...
if __name__=='__main__':
    ...
    if not os.path.isdir(args.output_dir):
        ...
        assert not any(
                        entry for entry in os.scandir(args.output_dir)
                        if not entry.name.startswith('.')
                    ), f"Error: Output folder {args.output_dir} is not empty!"
    ...
    if not os.path.isfile(score_file_path):
        with open(os.path.join(args.config_dir,f"prompts.json")) as f:
            config = json.load(f)
        if not os.path.isdir(args.output_dir):
            os.makedirs(args.output_dir)
        ...
        if args.encode_batch_size>0:
            ...
            kwargs.update({'encode_batch_size': args.encode_batch_size})
